...
def compute( df):
    ...
    for i in range(len(df)):
        ...
        forwardSlash = urlstr.find("//")
        print(forwardSlash)
        url = urlstr[forwardSlash+2:].split("/")
        #url = urlstr[10:].split("/")
        list1.append(url)
...
def computeLater( df):
    ...
    return (sred[-50:]) #TODO: CHANGE THE NUMBER OF LINKS RETRIEVEDD IF NEEDED TO
...
data = pd.read_csv('dataset2.txt',header = None, sep=' ')# or ' ' or '   ' or '    ' or '     ' or '      ' or '       ' or '        ' or '         ' or '          ' or '          '
